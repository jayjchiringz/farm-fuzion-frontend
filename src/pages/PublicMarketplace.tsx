...
export default function PublicMarketplace() {
  ...
  const AnalyticsTab = () => (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Market Intelligence</h2>
        <p className="text-gray-600 dark:text-gray-400">Real-time agricultural market prices and trends</p>
      </div>

      {/* Market Summary Stats */}
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard label="Products Tracked" value={stats.total_products.toString()} icon={<Package size={20} />} color="from-blue-500 to-blue-600" />
          <StatCard label="Cooperatives" value={stats.total_cooperatives.toString()} icon={<Building2 size={20} />} color="from-purple-500 to-purple-600" />
          <StatCard label="Global Orders" value={stats.total_orders.toString()} icon={<Truck size={20} />} color="from-orange-500 to-orange-600" />
          <StatCard label="Categories" value={categories.length.toString()} icon={<TrendingUp size={20} />} color="from-green-500 to-green-600" />
        </div>
      )}
          ...
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-gray-900/50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Price (KES/unit)</th>
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {marketPrices.slice(0, 10).map((price, idx) => (
                <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-900/30">
                  <td className="px-6 py-4 font-medium">{price.product_name}</td>
                  <td className="px-6 py-4">
                    <span className="font-bold text-brand-green">{formatKES(price.retail_price)}</span>
                    <span className="text-xs text-gray-500 ml-1">/{price.unit || 'kg'}</span>
                  </td>
                  <td className="px-6 py-4">
                    {price.trend === 'UP' && <span className="flex items-center gap-1 text-green-600"><ArrowUp size={14} /> +{price.weekly_change?.toFixed(1)}%</span>}
                    {price.trend === 'DOWN' && <span className="flex items-center gap-1 text-red-600"><ArrowDown size={14} /> {price.weekly_change?.toFixed(1)}%</span>}
                    {(!price.trend || price.trend === 'STABLE') && <span className="text-gray-500">Stable</span>}
                  </td>
                  <td className="px-6 py-4 text-gray-500">{price.region || 'National'}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-gray-800 dark:to-gray-900 p-4 rounded-xl border border-green-100 dark:border-gray-700">
          <h3 className="font-semibold text-green-800 dark:text-green-300 mb-2 flex items-center gap-2"><TrendingUp size={18} /> Best Time to Sell</h3>
          <p className="text-sm">Prices typically peak during harvest season (July-September). Consider holding produce for better returns.</p>
        </div>
        <div className="bg-gradient-to-br from-orange-50 to-yellow-50 dark:from-gray-800 dark:to-gray-900 p-4 rounded-xl border border-orange-100 dark:border-gray-700">
          <h3 className="font-semibold text-orange-800 dark:text-orange-300 mb-2 flex items-center gap-2"><TrendingDown size={18} /> Buyer's Market</h3>
          <p className="text-sm">Best time to buy is during peak harvest (January-March). Prices are typically 15-20% lower.</p>
        </div>
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-900 p-4 rounded-xl border border-blue-100 dark:border-gray-700">
          <h3 className="font-semibold text-blue-800 dark:text-blue-300 mb-2 flex items-center gap-2"><Activity size={18} /> Market Outlook</h3>
          <p className="text-sm">Stable demand expected. International buyers showing increased interest in Kenyan organic produce.</p>
        </div>
      </div>
    </div>
  );
    ...
    <MainLayout>
      ...
      <div className="flex min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
        ...
        >
              ...
              <div className="transition-all duration-500">
                {isSidebarOpen ? (
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xl font-light text-white/90 tracking-wide">Global Market</span>
                  </div>
                ) : (
          ...
          {stats && isSidebarOpen && (
            <div className="relative z-10 mb-4 p-3 bg-white/10 rounded-xl backdrop-blur-sm">
              <p className="text-xs text-white/70 mb-2">Global Marketplace</p>
          ...
          <div className="relative z-10">
            ...
            >
              ...
              {isSidebarOpen && <span className="text-sm font-medium">Sign In</span>}
            </button>
          </div>
        </aside>
        ...
        <main className="flex-1 overflow-y-auto">
          ...
          <div className="sticky top-0 z-30 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border-b border-gray-200 dark:border-gray-800 px-6 py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="relative">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-r from-brand-green to-green-600 flex items-center justify-center">
                    <Globe size={20} className="text-white" />
                  </div>
                </div>
                <div>
                  <h1 className="text-xl font-bold text-gray-900 dark:text-white">Global Marketplace</h1>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Buy directly from Kenyan cooperatives</p>
                </div>
              </div>
              <ThemeToggle />
            </div>
          ...
          <div className="p-6">
            {/* Hero Section */}
            <div className="bg-gradient-to-r from-brand-green to-green-700 rounded-2xl p-6 text-white mb-8 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -mr-20 -mt-20"></div>
              <div className="absolute bottom-0 left-0 w-48 h-48 bg-white/10 rounded-full -ml-16 -mb-16"></div>
              <div className="relative z-10">
                <h1 className="text-3xl font-bold mb-2">FarmFuzion Global Agro-Marketplace</h1>
                <p className="text-white/90 max-w-2xl">
                  Connect directly with Kenyan cooperatives. Buy fresh produce, grains, and agricultural products in bulk.
                </p>
                <div className="flex flex-wrap gap-3 mt-4">
                  <div className="flex items-center gap-2 bg-white/20 rounded-full px-3 py-1.5"><Leaf size={14} /><span className="text-sm">Certified Organic</span></div>
                  <div className="flex items-center gap-2 bg-white/20 rounded-full px-3 py-1.5"><Truck size={14} /><span className="text-sm">Bulk Shipping</span></div>
                  <div className="flex items-center gap-2 bg-white/20 rounded-full px-3 py-1.5"><Shield size={14} /><span className="text-sm">Verified Suppliers</span></div>
                </div>
              </div>
            </div>
